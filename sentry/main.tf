...
resource "yandex_compute_instance" "sentry" {
...
output "public_ip" {
  ...
  value       = yandex_compute_instance.sentry.network_interface.0.nat_ip_address
}
  ...
  content = templatefile("inventory_yml.tpl",
    {
      ...
      hostname  = var.hostname
      public_ip = yandex_compute_instance.sentry.network_interface.0.nat_ip_address
      domain    = var.domain
    }
  )
