...
resource "yandex_compute_instance" "jaeger" {
...
output "public_ip" {
  ...
  value       = yandex_compute_instance.jaeger.network_interface.0.nat_ip_address
}
...
resource "local_file" "inventory_yml" {
  content = templatefile("inventory_yml.tmpl",
    ...
    hostname            = var.hostname
    public_ip           = yandex_compute_instance.jaeger.network_interface.0.nat_ip_address
    domain              = var.domain
    }
  )
